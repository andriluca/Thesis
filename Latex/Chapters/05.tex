...
\Cref{fig:data_pipeline}. We extract a 3D surface mesh of lung lobes
...
\item \textbf{Mathematical and Electrical Components}.  The simplest
  ...
  «\texttt{ModelingToolkitStandardLibrary}»), while integral-dependent
  ...
  \item \emph{Current Integrator}: it computes the current integral
    and manages integration timing by means of a Callback function
    (see aforementioned Callbacks Section).
  \item \emph{Current Integral-Dependent Inductor}: It takes the
    current integral value from Current Integrator and computes the
