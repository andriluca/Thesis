...
** [#C] Compila AVATree da sorgente
Ho tentato in tutti i modi ma non è stato possibile in quanto
compaiono molti errori in fase di compilazione, si veda la pagina del
progetto su GitLab.
...
** DONE [#C] Comprendere le questioni legate alla licenza software che devo utilizzare
La licenza d'uso per Chaste è inclusa direttamente nello User Project
implementato, si veda sul mio GitHub la repository
`NewbornAirwayGeneration`.  Per quanto riguarda la licenza del
software scritto in Julia, devo chiedere al Professore ma da quanto ho
capito è un discorso che andrà fatto più avanti, in una fase più
...
** DONE Bootstrap di un test (Chaste)
...
*** DONE Raccogliere le domande da porgli
...
(tra i preferiti)
*** DONE [#A] Implementare triggers nello switch
...
** DONE Modello integratore di corrente
È possibile creare un componente a basso livello che gestisca
l'integrazione della corrente?  Esiste un nodo `Integrator` in
`continuous.jl` (un sorgente della libreria standard).  Posso
utilizzare questo all'interno dei circuiti?

#+begin_src julia
  @mtkmodel Integrator begin
      @components begin
          what = SISO(y_start=0)
          when = SISO(y_start=false)
      end
      @variables begin
      ...
      @equations begin
          # u[1]    -> corrente.
          # u[2]    -> trigger_in.
          # y[1], x -> integrale della corrente.
          # y[2]    -> trigger_out.

          # Quando l'integrale 1) sta per essere negativo, 2) sta per
          # superare V_FRC 3) trigger_in (alias per u[2]) è false,
          # l'integrazione non viene effettuata.

          D(x) ~ ifelse((((x < 0) & (what.u < 0)) | (x >= V_FRC) | (when.u == false)),
                        0,
                        what.u)
...
* 02.05.24
** Variabili booleane
È possibile utilizzare delle variabili booleane all'interno dei
componenti, in particolar modo nella macro `@equations` con ifelse. È
possibile fare anche la derivata di una variabile booleana e
utilizzare i continuous events, quindi anche le callbacks.
* 06.05.24
** TODO Provare a estrarre il vtp con Windows
Ho testato la libreria `vtkToolbox` per MATLAB su Windows 11 ma,
nonostante questa funzioni per il file .vtu, questa crasha quando devo
importare un file .vtp appena esportato da Slicer.

** DONE Riportare la struttura di `prova2.jl` nella libreria
Ho creato dei files chiamati `Lx*_<nome>` dove x è un numero che
indica il livello dei moduli (minore il livello, più alto il suo
ordine). La logica che seguo è quella di una piramide. Il problema è
che la simulazione del sistema risulta essere instabile in questo
modo.  Devo cercare di capire perché.
** TODO Implementazione diodo

* 07.05.24
** DONE Scrivere mail a Chiara
#+begin_example
Ciao Chiara, come stai?
Ti aggiorno su quello che ho fatto.  Ho provato ad utilizzare `vtkToolbox` su Matlab e con il file `major_airways.vtu` (quello che vorremmo ottenere e compatibile con Chaste) l'importazione funzionava correttamente col metodo che ci aveva detto Davide.  Il problema però si verifica quando vado ad importare il file vtp generato da Slicer poiché la sua importazione causa il crash di Matlab.  Ho provato parecchie volte e con diverse versioni di Matlab e ho ottenuto lo stesso risultato ogni volta.  Ho quindi riguardato la codebase della libreria che sto scrivendo e l'ho organizzata in maniera gerarchica per avere in ogni file dei componenti di ordine via via superiore e ho implementato il codice di un integratore che opera una sola volta (per alveolo o via aerea) l'integrazione e gestisce il timing delle flag `trigger_in` e `trigger_out`.  Ho provato la formula che mi avevi inviato su Whatsapp e mi crea lo stesso dei problemi di stabilità nello switch ma posso ritentare con altri valori.  Ho quindi implementato una mia versione dello switch con questa equazione: R = Rlow + (trigger_in - trigger_out) * (Rhigh - Rlow) e sembra funzionare con qualche modulo modificando la dimensione del differenziale di tempo.  Il diodo ancora non riesco a farlo funzionare correttamente e volevo chiederti se avevi dei suggerimenti da propormi perché ho fatto un po' di tentativi ma non riesco a "bloccare" la corrente.
#+end_example

* 09.05.24
** TODO Modificare lo script `cir2jl`
Con la nuova stuttura modulare è necessario che questo script agisca
per generare un file L1_lungs.  Questo file contiene componenti
(alveoli e vie aeree) ed equazioni di due tipi: connect() per le
connessioni tra moduli ed equazioni algebriche per gestire il timing
dell'integrazione.  Nella radice dell'albero respiratorio ho che il
trigger_in è true (l'integrazione comincia a t = 0).  È possibile
ottimizzare lo script?  Innanzitutto devo invertire l'ordine degli
input e output delle connessioni.  Nella creazione del modello di
sistema devo eliminare qualsiasi riferimento ai parametri di
simulazione.  Sempre in questa sezione devo modificare il
connect(source.p, $2.in) utilizzando il pin di input.  Sembra più
complesso di quanto pensassi: cerco i moduli che ho utilizzato nella
sottorete e vedo di utilizzare lo script su quel file cir.  C'è un
problema: dal file `Albero.cir` non si riesce a capire come collegare
la rete.  Devo chiedere a Chiara.

** DONE [#A] Chiedere a Chiara come ottenere informazioni sulla rete da file cir
Prima di chiedere consultare il video che Chiara mi aveva mandato su
Whatsapp.

Penso di aver capito.  Data una via aerea, il cui nome è all'inizio
della riga del cir, devo ricordarmi gli ultimi due parametri
all'interno delle parentesi e per trovare quali altre vie sono
connesse a questa via aerea devo fare il grep su tutto il documento di
questi parametri ma invertiti di posizione.

Devo:
1) Estrarre la prima riga.
   #+begin_src bash
     row=$(head -1 Verilog/Albero_sottorete.cir | tr -d '\r()')
   #+end_src

2) Estrarre la 1ª colonna per il nome.
   #+begin_src bash
     name=$(echo $row | cut -d ' ' -f1)
   #+end_src

3) Considerare la 5ª colonna.
   #+begin_src bash
     output_node=$(echo $row | cut -d ' ' -f5)
     pattern=$(echo "$output_node N")
   #+end_src
   
4) Cercare le righe in cui la variabile è contenuta e stampo il primo elemento.
   #+begin_src bash
     inputs=$(grep "$pattern" Verilog/Albero_sottorete.cir | cut -d ' ' -f1)
     connect_inputs=$(for i in $inputs; do echo "$i.in,"; done | tr '\n' ' ')
     timings=$(for i in $inputs; do echo "$i.i1.trigger.u"; done)
   #+end_src

5) Costruisco la riga per il file.
   #+begin_src bash
     out_connect=$(echo "connect($nome.out, $connect_inputs)")
     out_timings=$(for i in $timings; do echo "$i ~ $name.i1.trigger.y"; done)
   #+end_src
   
** DONE [#A] Includo questo snippet in `cir2jl`
Questo codice funziona

#+begin_src bash
  rm tmp;

  tail -n +2 Verilog/Albero.cir > Verilog/Albero_tagliato.cir

  while IFS= read -r line;
  do
      row=$(echo "$line" | tr -d '\r()');
      name=$(echo $row | cut -d ' ' -f1)
      output_node=$(echo $row | cut -d ' ' -f5)
      pattern=$(echo "$output_node N")
      inputs=$(grep "$pattern" Verilog/Albero_tagliato.cir | cut -d ' ' -f1)
      connect_inputs=$(for i in $inputs; do echo "$i.in,"; done | tr '\n' ' ')
      timings=$(for i in $inputs; do echo "$i.i1.trigger.u"; done)
      [[ ! -z $connect_inputs ]] && echo "connect($name.out, $connect_inputs)" >> tmp
      for i in $timings;
      do
          [[ ! -z $i ]] && echo "$i ~ $name.i1.trigger.y" >> tmp;
      done
  done < "Verilog/Albero_tagliato.cir"
#+end_src

* 10.05.24

Prima di dedicarmi alla sezione dello script deputata alla generazione
del codice dei moduli è necessario ultimare quella che stavo scrivendo
ieri inserendo anche la connessione iniziale.

#+begin_src bash
  tail -n +2 Verilog/Albero_sottorete.cir > Verilog/Albero_tagliato.cir

  echo "connect(in, $2)" > tmp
  echo "$2.i1.trigger.u ~ $name.i1.trigger.y" >> tmp
  ...
  do
      ...
      [[ ! -z $connect_inputs ]] && echo "connect($node_name.out, $connect_inputs)" >> tmp

      for i in $timings;
      do
          [[ ! -z $i ]] && echo "$i ~ $node_name.i1.trigger.y" >> tmp;
      done

  done < "Verilog/Albero_tagliato.cir"
#+end_src

Non è stato necessario lavorare sul codice per generare i moduli
perché era già stato scritto in precedenza.  Ho generato il file da
`Albero.cir` e sto provando ad importarlo su Julia.

* 13.05.24
** Computazione remota
Posso utilizzare un server remoto per effettuare la computazione.

| Sistema      | Caratteristiche                 |
|--------------+---------------------------------|
| Produttore   | HP                              |
| Processore   | AMD EPYC 7351 16-Core Processor |
| RAM          | 16 GB                           |
| Tipo sistema | 64bit                           |
| OS           | Windows Server 2019 Standard    |

Per ovvi motivi non posso condividere credenziali di accesso.  Posso
utilizzarlo in orari serali/notturni.

** Diodo
Ci sono già stati in passato dei tentativi di implementazione di
questo componente ma non hanno portato a buoni risultati. provo a
ripensare la logica di questo componente a partire da un circuito i
cui andamenti di corrente e tensione sono noti.

Il comportamento che desidererei ottenere è quello di una resistenza
che varia al variare della tensione ai suoi capi.  Quando la tensione
è al di sotto di un valore di soglia, devo impedire il passaggio della
corrente il più possibile.  Questo significa che la resistenza dovrà
essere elevata per valori di tensione inferiori alla soglia.  Quando
la tensione invece si avvicina a quella di threshold allora la
corrente che dovrà scorrere deve necessariamente aumentare, il che
significa una minore resistenza.
...
Questo codice crea problemi nel punto in cui la resistenza cambia.

* 27.05.24
Francesca ha fornito una versione del file vtu dei primi tratti delle
vie aeree.  Il prossimo step che devo provare a fare è quello di
creare uno user project plasmato sul test chiamato
`TestAirwayGenerationTutorial`.

* 30.05.24
In questi giorni ho letto la documentazione di Chaste e vi sono delle
classi per manipolare le mesh.  `TetrahedralMesh`, in particolare, è
quella che viene utilizzata da `TestAirwayGenerationTutorial`.
`TetrahedralMesh` viene popolata tramite un metodo
`.ConstructFromMeshReader`.  È possibile fare la stessa cosa ma senza
utilizzare il vtu? quindi a partire da file `TestSubject002.node` e
`TestSubject002.edge`?

* 03.06.24
Ho avuto contatti con un radiologo interno al FBF che potrebbe fornire
dei dati anonimizzati di TC neonatale ma lui consiglia di utilizzare
una tac di neonato a termine o di bambino.  Devo chiedere a Chiara se
è necessaria una tac del genere o se non lo è.

Devo dire a Francesca che ho trovato la classe `TrianglesMeshReader`
con la quale riesco a leggere dei file `.node` e `.edge` tramite i
quali posso esplicitare la mesh senza passare per il formato `vtu`.

Devo dire a Chiara che il risultato dei Diodi non è quello che
vorremmo ottenere in termini di corrente.  Posso mostrarle dei grafici
di corrente e tensione col circuito semplice R-D-C.

* 05.06.24
Devo sistemare il test su Ubuntu prima di tutto ma per farlo devo
sincronizzare la repository locale (NewbornAirwayGeneration) sul
computer di Chiara con quella remota dedicata allo User Project.

* 07.06.24
Ho parlato con Chiara e Francesca.  Mi è stato detto che dobbiamo
lavorare sui file testuali che Chaste vuole in input per generare
l'albero respiratorio.

* 10.06.24
Devo mandare una mail a Chiara per capire che tipo di TC vuole per
fare una valutazione con diverse età gestazionali.  Se le interessa
quella di un neonato a termine oppure bambino.  La risposta è stata
neonato a termine e prematuro.

Per quanto riguarda la scrittura, devo scaricare i documenti inviatimi
da Chiara, convertirli in PDF e metterli in una cartella che si
sincronizza tra vari dispositivi (`~/Papers` ad esempio).

* 11.06.24
** DONE Prova Chaste
Prova Chaste con i file in `Downloads` su questo pc (partizione
Ubuntu).
* 12.06.24
Sono stati modificati i file di Francesca e ho ottenuto un primo
risultato paziente-specifico.  Questa è la prima volta che ottengo un
risultato del genere.  Ho salvato i file nella Repo.

Siccome i primi tratti dell'albero respiratorio non erano completi ho
riscritto il file `airways.edge` a mano per effettuare manualmente il
collegamento tra un nodo e l'altro.

* 13.06.24
Ho mostrato i risultati relativi all'anatomia a Chiara e sembrano fare
al caso nostro.  Per ora tengo in considerazione quelli che ho già
generato ma sarà necessario in futuro aggiornare i parametri di
generazione.  Dall'output è possibile ricavare importanti dati
relativi alla rete complessiva, alla geometria ecc.

Posso cominciare intanto a scrivere la tesi! 😊
